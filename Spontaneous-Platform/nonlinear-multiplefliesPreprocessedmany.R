...
prediction = nonLinearPredict (use.ts[1:round(length(use.ts)/2)], embedding.dim = emb.dim, time.lag = LAG, radius = 0.0005,
                                radius.increment = 0.001, prediction.step = 1:200)
# comparamos... Fijate como la prediccion va empeorando a medida que pasa el tiempo. Esto
# es tipico de sistemas no lineales

plot(use.ts[(round(length(use.ts)/2)+1):(round(length(use.ts)/2)+200)], type = "l", ylim = range(use.ts[(round(length(use.ts)/2)+1):(round(length(use.ts)/2)+200)], Prediction))
lines(prediction, type = "l", col = 2)

Observed<-c(Observed,use.ts[(round(length(use.ts)/2)+1):(round(length(use.ts)/2)+200)])
...
ifelse(i==1, Params <- data.frame("Fly name"=diffdata[i], "Corr Dim"=D,"Lyapunov Exp"=ml.estimation,"Sample entropy estimate"=EstimatedEntropy,"detrend fluctuation analysis"=detr.fluct, "Surrogate statistic"=st$data.statistic,
       ...
       , Params[i,1:ncol(Params)] <- c(diffdata[i], D,ml.estimation,EstimatedEntropy,detr.fluct,st$data.statistic, regression.range.min , regression.range.max , use.embeddings.min, use.embeddings.max, theilerw))
