...
findAllNeighbours(takens, radius, number.boxes = NULL)                      #from nonlinearTseries. This one needs epsilon but this isn´t still calculated.

estimateEmbeddingDim(time.series, number.points = length(time.series),                       #from nonlinearTseries
...
Takens= buildTakens(derivative, embedding.dim, time.lag=LAG)
...
embedding<-embedd(derivative,5,1)
plot(embedding)

poincareMap(takens = Takens)     # from nonlinearTseries
...
d2(derivative, m=20, d=1, t=10, eps.min=0.1, neps=20)
...
correlDim<- corrDim(time.series, min.embedding.dim = 2, max.embedding.dim = 5,time.lag = LAG, min.radius, max.radius, corr.order = 2,
            n.points.radius = 5, theiler.window = 100, do.plot = TRUE,number.boxes = NULL, ...)
...
# I have to see what infDim does, what is the difference with the above

infD<-infDim(time.series, min.embedding.dim = 2,
       max.embedding.dim = min.embedding.dim, time.lag = 1, min.fixed.mass,
       max.fixed.mass, number.fixed.mass.points = 10, radius,
       increasing.radius.factor = sqrt(2), number.boxes = NULL,
       number.reference.vectors = 5000, theiler.window = 1, kMax = 1000,
       do.plot = TRUE, ...)
