...
message("please enter the parameters for the estimation of the correlation dimension")
use.embeddings.min = scan(n=1,what= numeric())
message("please enter the parameters for the estimation of the correlation dimension")
use.embeddings.max = scan(n=1,what= numeric())

output<- paste(id_table1[i],"Interpolated.pdf", sep="")
pdf(file= output)
...
prediction = nonLinearPredict (use.ts[1:round(length(use.ts)/2)], embedding.dim = emb.dim, time.lag = LAG, radius = 0.02,
                                radius.increment = 0.05, prediction.step = 1:2000)
# comparamos... Fijate como la prediccion va empeorando a medida que pasa el tiempo. Esto
# es tipico de sistemas no lineales
plot(use.ts[(round(length(use.ts)/2)+1):(round(length(use.ts)/2)+2000)], type = "l", ylim = range(use.ts[(round(length(use.ts)/2)+1):(round(length(use.ts)/2)+2000)], Prediction))
